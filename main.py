    ...
    if balance >= price != 0:
        if limit != 0:
            ...
            bot.send_message(client_code_moshtari,
                             f"لینک بالا برای استفاده روی سیستم عامل های Android و ios میباشد اگر فایل همین کانفیگ رو برای استفاده در ویندوز میخاهید به پشتیبانی مراجعه کنید\n نام کاربری شما:<code>{client_code_moshtari}</code> \n  روی شماره کاربریت کلیک کن و مستقیم به آیدی زیر ارسال کن. \n آیدی:@jimboovpn_Support",
                             parse_mode="HTML",
...
def handle_message(message):
    ...
    if message.text == "خرید اشتراک🛍":
        ...
        bot.send_message(message.chat.id,
                         f"<code>برای برداشت موجودی داشبورد خود، نام کاربری خودتون رو : {message.chat.id}</code> برای پشتیبان ارسال نمایید  آیدی :@jimboovpn_Support")
